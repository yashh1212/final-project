...
def create_invoice(patient_name, appointment_id, payment_status, total_amount, payment_id, hospital_name,
                   address_hospital, doctor_name, gender, appointment_date, appointment_schedule, phone_number,
                   patient_age):
    pdf = PDF()
    pdf.add_page()
    global pdf_name
    ...
    if (flag == False):
        if (app_id1 is not None):
            if send_details_via_email():
                if create_invoice(pt_name, app_id1, payment_status, paid_amount, payment_id, hospital, address_hospital,
                                  doctor_name, gender, appointment_date, appointment_schedule, phone_number,
                                  patient_age):

                    if cursor.execute(
                            "INSERT INTO patient_appointment (patient_name, patient_age, patient_no, doctor_name, appointment_date, appointment_schedule, payment_status, patient_gender,appointment_id) VALUES (%s, %s, %s, %s, %s, %s,%s, %s, %s)",
                            (pt_name, DOB, pt_phone, doctor_name, date, app_time, payment, pt_gender, app_id1)):
                        conn.commit()
                        flag = session['flag'] = True
                        return render_template('paymentdetail.html', pt_name=pt_name, Dr_id=Doctor_id,
                                               appointment_id=app_id1, Dr_name=doctor_name, Appoinment_date=date,
                                               hospital_address=address_hspital, Appoinment_time=app_time,
                                               pdf_name=pdf_name)
        else:
...
def appointmentdeatil():
    try:
        ...
        if cursor.execute(
                "INSERT INTO appointment_info (paient_name, patient_email, phone, pt_gender, dob, appointment_date, app_time) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
def detailconfirm():
    ...
    return render_template("detailconfirm.html", Address=address, hospital=hospital, doctor_name=doctor_name,
                           head_name=head_name, pt_firstname=pt_firstname, pt_lastname=pt_lastname, pt_email=pt_email,
                           pt_phone=pt_phone, DOB=DOB, date=date, app_time=app_time, pt_gender=pt_gender)
...
def searchdoctor():
    try:
        ...
        cursor.execute(
            "SELECT fullname, d_email, d_number, experience, qualification, specialist, city, id FROM login_data_doctor WHERE specialist = %s and city = %s",
            (specialty, city))
...
def add_user():
    ...
    if hashed_password == bcrypt.hashpw(cpassword.encode('utf-8'), hashed_password):
        ...
        cursor.execute(
            "INSERT INTO login__data (firstname, lastname, email, mobile_no, password, gender) VALUES (%s, %s, %s, %s, %s, %s)",
...
def add_doctor():
    ...
    if d_hashed_password == bcrypt.hashpw(c_pass.encode('utf-8'), d_hashed_password):
        ...
        cursor.execute(
            "INSERT INTO login_data_doctor (fullname, d_email, d_number, experience, qualification, specialist, city, d_gender, password, hospital, address) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
            (f_name, d_email, d_number, yoe, qualification, specialist, city, gender, d_hashed_password, hospital_name,
             hospital_address))
        conn.commit()
...
def doctor_home():
    ...
    heading = ('patient_name', 'patient_age', 'patient_no', 'doctor_name', 'appointment_date', 'appointment_schedule',
               'payment_status', 'appointment_id', 'Gender')
    data = user
